...
# %% Description
#
# This script generates spatially and temporally correlated noise and
...
if outputpath == "/path/to/noise/":
    raise ValueError(
        "ERROR: Please set 'outputpath' to your own output directory path!"
    )

dt = 3  # time interval --> 3 hours for ERA5 data --> please adjust
...
rho = 7 / 8  # decorrelation time 24h. time step of forcings 3h --> decorrelation after
# one days which means 3h are 1/8 --> 7/8 decorrelation time step
...
# standard deviations and mean, for precipitation and short wave radiations,
# log normal distributions are there.
...
if rnd_state_file == "/path/to/random/state/rnd_state.json":
    raise ValueError(
        "ERROR: Please set 'rnd_state_file' to your own random state file path!"
    )
...
# %% grid of input data (not real longitude and latitude but only grid on which the
# %% noise is simulated)
...
# %% generate Matrix for spatial correlation, distances between all grid points have to
# %% be computed
...
for year, month in zip(years, months * len(years)):
    ...
    fname = "/path/to/forcings/era5/" + str(year) + "-" + str(month).zfill(2) + ".nc"
    if fname.startswith("/path/to/forcings/era5/"):
        raise ValueError(
            "ERROR: Please update the forcing data path in the loop to your directory!"
        )
